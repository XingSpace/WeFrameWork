...
public class MyWindowUtil {
    ...
    /**
     * 用于嵌套如ScrollView 动态设置GridView的高度
     *
     * @param gridView
     * @param clum
     * @param context
    ...
    public static void setGridViewHeightBasedOnChildren(GridView gridView,
                                                        int clum, Context context, float h) {
        if (gridView == null)
            return;
        ...
        int totalHeight = 0;
        int count = listAdapter.getCount() / clum;
        if (listAdapter.getCount() % clum > 0) {
            count += 1;
        } else {
        ...
        int a = gridView.getHorizontalSpacing();
        //	Log.e("", a + "");
        params.height = totalHeight
                + (DensityUtils.dp2px(context, h) * (count - 1));
